...
Updating k8s is a special case, for that one needs to update the client-go
library first and then the remaining dependencies:
...
Lets assume we want to add a new Kubernetes version ``v1.19.0``:

#. Follow the above instructions to update the Kubernetes libraries.

#. Follow the next instructions depending if it is a minor update or a patch
   update.
...
#. Open all files in the ``jenkinsfiles/`` directory, and bump all versions
   ...
   is running with the new Kubernetes version by default.
